...
There are three types of steps you can write on Docket:
...
Random alphanumeric variables generate unique random strings (letters and numbers) for each test run. You can set the length and toggle between uppercase/lowercase letters. Perfect for unique usernames, product IDs, or any data that needs to be different every time.
