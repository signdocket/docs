...
- **Auto-sync**: Changes to test configuration (variables, files, and logged-in user session) are automatically synced to the browser. A resync is only needed when changing viewport size, zoom level, or when the underlying browser needs to be restarted. Changing the entrypoint URL will prompt you with an option to navigate to the new URL
...
#### Resync Browser
Opens a resync dialog with two options:
